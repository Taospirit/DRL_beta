...
class SAC(BasePolicy):
    ...
    def choose_action(self, state, test=False):
        # state = torch.tensor(state, dtype=torch.float32, device=self.device)
        action = self.actor_eval.choose_action(state, test)
    ...
    def learn(self):
        ...
        pg_loss /= self._update_iteration
        v_loss /= self._update_iteration
        q_loss /= self._update_iteration
        return pg_loss, v_loss, q_loss